<?php
$title = 'Balanço de Embarques - Análise Logística';
$content = '
<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h2 text-primary">
                <i class="bi bi-calculator"></i> Balanço de Embarques
            </h1>
            <div class="d-flex gap-2">
                <button class="btn btn-outline-primary" onclick="refreshBalanco()">
                    <i class="bi bi-arrow-clockwise"></i> Atualizar
                </button>
                <button class="btn btn-success" onclick="exportBalancoData()">
...
<div class="row mb-4" id="balancoStats" style="display: none;">
    <div class="col-xl-3 col-md-6 mb-3">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body text-center">
                ...
                <h5 class="card-title text-muted mb-1">Total de Mesorregiões</h5>
                <h2 class="text-primary mb-0" id="totalMesorregioes">-</h2>
            </div>
        </div>
    </div>
...
<div class="row mb-4">
    <div class="col-12">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-transparent border-0">
                <h5 class="card-title mb-0">
                    <i class="bi bi-graph-up"></i> Gráfico de Balanço por Mesorregião
                </h5>
            </div>
            <div class="card-body">
                <canvas id="balancoChart" height="100"></canvas>
            </div>
        </div>
    </div>
</div>
...
<div class="row mb-4">
    <div class="col-12">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-transparent border-0">
                <h5 class="card-title mb-0">
                    <i class="bi bi-table"></i> Tabela de Balanço
                </h5>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-hover" id="tabelaBalanco">
...
function showNoDataAlert() {
    ...
    document.getElementById("balancoStats").style.display = "none";
}

// Ocultar alerta de dados não carregados
function hideNoDataAlert() {
    document.getElementById("noDataAlert").classList.add("d-none");
    document.getElementById("balancoStats").style.display = "flex";
}

// Carregar dados do balanço
function loadBalancoData() {
    console.log("Carregando dados do balanço com filtros:", currentFilters);
    ...
    fetch(`/api/balanco_embarques?${params.toString()}`)
        .then(response => response.json())
        .then(data => {
            console.log("Dados do balanço recebidos:", data);
            ...
            balancoData = data;
            renderBalanco(data);
            updateBalancoStats(data);
            createBalancoChart(data);
        })
        .catch(error => {
...
function renderBalanco(data) {
    if (!data || !data.balanco) {
        console.error("Dados do balanço inválidos");
        return;
    }
    
    const tbody = document.querySelector("#tabelaBalanco tbody");
    if (!tbody) {
...
// Atualizar estatísticas do balanço
function updateBalancoStats(data) {
    if (!data || !data.balanco) return;
    
    const balanco = data.balanco;
    const totalMesorregioes = balanco.length;
    ...
    document.getElementById("totalMesorregioes").textContent = totalMesorregioes.toLocaleString("pt-BR");
...
function createBalancoChart(data) {
    if (!data || !data.balanco) return;
    ...
    const balanco = data.balanco;
    const labels = balanco.map(item => item.mesorregiao);
    ...
    try {
        ...
        console.log("Gráfico de balanço criado com sucesso");
...
// Exportar dados do balanço
function exportBalancoData() {
...
// Mostrar erro do balanço
function showBalancoError() {
    const tbody = document.querySelector("#tabelaBalanco tbody");
    tbody.innerHTML = `
        <tr>
            <td colspan="6" class="text-center text-muted py-4">
